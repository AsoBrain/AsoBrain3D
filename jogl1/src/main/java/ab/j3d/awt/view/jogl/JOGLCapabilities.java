...
public class JOGLCapabilities
{
	...
	extends Probe
	{
		...
		protected void run( final GL gl )
		{
			...
			if ( _shaderObjects )
			{
				_maxVaryingFloats = getInteger( gl, GL.GL_MAX_VARYING_FLOATS );
				_maxCombinedTextureImageUnits = getInteger( gl, GL.GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS );
				_maxTextureImageUnits = getInteger( gl, GL.GL_MAX_TEXTURE_IMAGE_UNITS );
				_maxFragmentUniformComponents = getInteger( gl, GL.GL_MAX_FRAGMENT_UNIFORM_COMPONENTS );
				_shadingLanguageVersion = gl.glGetString( GL.GL_SHADING_LANGUAGE_VERSION );
			}
			else if ( _shaderObjectsARB )
			{
				_maxVaryingFloats = getInteger( gl, GL.GL_MAX_VARYING_FLOATS_ARB );
				_maxCombinedTextureImageUnits = getInteger( gl, GL.GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS_ARB );
				_maxTextureImageUnits = getInteger( gl, GL.GL_MAX_TEXTURE_IMAGE_UNITS_ARB );
				_maxFragmentUniformComponents = getInteger( gl, GL.GL_MAX_FRAGMENT_UNIFORM_COMPONENTS_ARB );
	...
	extends Probe
	{
		...
		protected void run( final GL gl )
		{
			if ( isShaderSupported() )
			{
				...
				final TextureCache textureCache = new TextureCache( null );
