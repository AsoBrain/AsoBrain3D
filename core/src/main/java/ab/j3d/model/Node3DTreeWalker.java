...
public class Node3DTreeWalker
{
	...
	{
		final Node3DTreeWalker treeWalker = new Node3DTreeWalker();
		return treeWalker.walkNode( visitor, Matrix3D.IDENTITY, node );
	}
	...
	{
		final Node3DTreeWalker treeWalker = new Node3DTreeWalker();
		return treeWalker.walkNode( visitor, transform, node );
	}
	...
	{
		final Node3DTreeWalker treeWalker = new Node3DTreeWalker();
		return treeWalker.walkNode( visitor, path );
	}

	/**
	 * Constructs a new instance.
	 */
	public Node3DTreeWalker()
	{
	}
	 ...
	 *          <code>false</code> if the tree walk was aborted.
	 */
	public boolean walkNode( @NotNull final Node3DVisitor visitor, @NotNull final Node3D node )
	{
		return walkNode( visitor, Matrix3D.IDENTITY, node );
	}
	 ...
	 *          <code>false</code> if the tree walk was aborted.
	 */
	public boolean walkNode( @NotNull final Node3DVisitor visitor, @NotNull final Matrix3D transform, @NotNull final Node3D node )
	{
		return walkNode( visitor, createPath( null, transform, node ) );
	}
	...
	public boolean walkNode( @NotNull final Node3DVisitor visitor, @NotNull final Node3DPath path )
	{
		...
		if ( result )
		{
			...
			{
				if ( !walkNode( visitor, createPath( path, transform, child ) ) )
				{
					result = false;
					break;
				}
	 ...
	 */
	@NotNull
	protected Node3DPath createPath( @Nullable final Node3DPath parentPath, @NotNull final Matrix3D currentTransform, @NotNull final Node3D node )
	{
	 ...
	 */
	@NotNull
	protected Matrix3D getCombinedTransform( @NotNull final Matrix3D currentTransform, @NotNull final Node3D node )
	{
		...
		if ( node instanceof Transform3D )
		{
			...
			if ( nodeTransform != Matrix3D.IDENTITY )
			{
