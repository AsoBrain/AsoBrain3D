...
vec4 lighting( in vec4 color )
{
	...
	// And from: http://www.gamedev.net/reference/articles/article2428.asp
	...
	for ( int i = 0 ; i < lightCount ; i++ )
	{
		if ( gl_LightSource[ i ].position.w == 0.0 )
		{
			...
			vec3 Iamb  = gl_LightSource[ i ].ambient .rgb;
			vec3 Idiff = gl_LightSource[ i ].diffuse .rgb *      max( dot( N , L ) , 0.0 );
			vec3 Ispec = gl_LightSource[ i ].specular.rgb * pow( max( dot( N , H ) , 0.0 ) , gl_FrontMaterial.shininess );

			material += Iamb + color.rgb * Idiff;
			specular += Ispec;
		}
		else
		{
			...
			vec3 Idiff = gl_LightSource      [ i ].diffuse .rgb *      max( dot( N , L ) , 0.0 );
			vec3 Ispec = gl_FrontLightProduct[ i ].specular.rgb * pow( max( dot( R , E ) , 0.0 ) , gl_FrontMaterial.shininess );
