...
public class JOGLTools
{
	...
	public static void paintObject3D( final GL gl, final Object3D object3D, final Matrix3D node2gl, final boolean fill, final Color fillColor, final boolean outline, final Color outlineColor, final boolean useAlternative )
	{
		...
		for ( int i = 0; i < object3D.getFaceCount(); i++ )
		{
			...
			if ( fill )
			{
				...
				{
					gl.glShadeModel(
					face.isSmooth() ? GL.GL_SMOOTH : GL.GL_FLAT );
	...
	/**
	 * Draw quad using GL with texture.
	 *
	 * @param gl       GL context.
	 * @param v1X      X coordinate of first vertex.
	 * @param v1Y      Y coordinate of first vertex.
	...
	public static void setMaterial( final GL gl, final Material material )
	{
		...
		material.ambientColorRed , material.ambientColorGreen ,
		material.ambientColorBlue , 1.0f
		}, 0 );
		gl.glMaterialfv( GL.GL_FRONT, GL.GL_DIFFUSE, new float[] {
		material.diffuseColorRed , material.diffuseColorGreen ,
		material.diffuseColorBlue , material.diffuseColorAlpha
		}, 0 );
		gl.glMaterialfv( GL.GL_FRONT, GL.GL_SPECULAR, new float[] {
		material.specularColorRed , material.specularColorGreen ,
		material.specularColorBlue , 1.0f
		}, 0 );
		gl.glMaterialfv( GL.GL_FRONT, GL.GL_EMISSION, new float[] {
		material.emissiveColorRed , material.emissiveColorGreen ,
		material.emissiveColorBlue , 1.0f
		}, 0 );
	}
