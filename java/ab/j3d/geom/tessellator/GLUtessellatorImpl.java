...
//                 MAX(sizeof(GLUvertex),sizeof(GLUface))))
    ...
    private GLUtessellatorImpl() {
        ...
        windingRule = GLUtessellator.GLU_TESS_WINDING_ODD;
        flagBoundary = false;
        boundaryOnly = false;
...
//        callMesh = NULL_CB;
        ...
        while (state != newState) {
            ...
            if (state < newState) {
                if (state == TessState.T_DORMANT) {
                    callErrorOrErrorData(GLUtessellator.GLU_TESS_MISSING_BEGIN_POLYGON);
                    gluTessBeginPolygon(null);
                } else if (state == TessState.T_IN_POLYGON) {
                    callErrorOrErrorData(GLUtessellator.GLU_TESS_MISSING_BEGIN_CONTOUR);
                    gluTessBeginContour();
                }
            } else {
                if (state == TessState.T_IN_CONTOUR) {
                    callErrorOrErrorData(GLUtessellator.GLU_TESS_MISSING_END_CONTOUR);
                    gluTessEndContour();
                } else if (state == TessState.T_IN_POLYGON) {
                    callErrorOrErrorData(GLUtessellator.GLU_TESS_MISSING_END_POLYGON);
    ...
    public void gluTessProperty(int which, double value) {
        switch (which) {
            case GLUtessellator.GLU_TESS_TOLERANCE:
                if (value < 0.0 || value > 1.0) break;
                relTolerance = value;
                return;

            case GLUtessellator.GLU_TESS_WINDING_RULE:
            ...
            case GLUtessellator.GLU_TESS_BOUNDARY_ONLY:
                boundaryOnly = (value != 0);
                return;

            case GLUtessellator.GLU_TESS_AVOID_DEGENERATE_TRIANGLES:
                avoidDegenerateTris = (value != 0);
                return;

            default:
                callErrorOrErrorData(GLUtessellator.GLU_INVALID_ENUM);
                return;
        }
        callErrorOrErrorData(GLUtessellator.GLU_INVALID_VALUE);
    }
...
/* tolerance should be in range [0..1] */
                ...
                break;
            case GLUtessellator.GLU_TESS_WINDING_RULE:
                assert (windingRule == GLUtessellator.GLU_TESS_WINDING_ODD ||
                        windingRule == GLUtessellator.GLU_TESS_WINDING_NONZERO ||
                        windingRule == GLUtessellator.GLU_TESS_WINDING_POSITIVE ||
                        windingRule == GLUtessellator.GLU_TESS_WINDING_NEGATIVE ||
                        windingRule == GLUtessellator.GLU_TESS_WINDING_ABS_GEQ_TWO);
    ...
    public void gluTessCallback(int which, GLUtessellatorCallback aCallback) {
        switch (which) {
            case GLUtessellator.GLU_TESS_BEGIN:
                callBegin = aCallback == null ? NULL_CB : aCallback;
                return;
            case GLUtessellator.GLU_TESS_BEGIN_DATA:
                callBeginData = aCallback == null ? NULL_CB : aCallback;
                return;
            case GLUtessellator.GLU_TESS_EDGE_FLAG:
 ...
 */
                ...
                return;
            case GLUtessellator.GLU_TESS_EDGE_FLAG_DATA:
 ...
 */
    ...
    public void gluTessVertex(double[] coords, int coords_offset, Object vertexData) {
        ...
        if (flushCacheOnNextVertex) {
            if (!flushCache()) {
                callErrorOrErrorData(GLUtessellator.GLU_OUT_OF_MEMORY);
                return;
            }
            lastEdge = null;
        }
        for (i = 0; i < 3; ++i) {
            ...
            if (x < -GLUtessellator.GLU_TESS_MAX_COORD) {
                x = -GLUtessellator.GLU_TESS_MAX_COORD;
                tooLarge = true;
            }
            if (x > GLUtessellator.GLU_TESS_MAX_COORD) {
                x = GLUtessellator.GLU_TESS_MAX_COORD;
        ...
        if (tooLarge) {
            callErrorOrErrorData(GLUtessellator.GLU_TESS_COORD_TOO_LARGE);
        }
